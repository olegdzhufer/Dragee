...
// =================== PACK FLAGS ===================
#define EB_CLKS_R (1 << 0)  // cliks released flag
#define EB_PRS_R (1 << 1)  // press released
#define EB_HLD_R (1 << 2) // hold release
#define EB_STP_R (1 << 3) // step release
#define EB_REL_R (1 << 4) // released 
#define EB_PRS (1 << 5) // press
#define EB_HLD (1 << 6) //hold
#define EB_STP (1 << 7) //step 
#define EB_REL (1 << 8)// release
#define EB_BUSY (1 << 9) // processing now
#define EB_DEB (1 << 10) // debounce
#define EB_TOUT (1 << 11) // timeout reached
#define EB_INV (1 << 12) // inverted -> on HIGH or LOW
#define EB_BOTH (1 << 13) // both buttons are pressed
#define EB_BISR (1 << 14) // pressed in interrupt
#define EB_EHLD (1 << 15) //holding now
#define EB_MODE (1 << 16) // pin mode IN/OUT // pin_mode(columnPin, OUTPUT);
...
class VirtButton {
   public:
    ...
    // установить pinmode кнопки (OUTPUT, INPUT)
    void setBtnMode(uint8_t mode){
        write_bf(EB_MODE, !mode);
    }
        ...
        if (s) {                                        // кнопка нажата
            ...
            if (read_bf(EB_PRS)) {                     // но была нажата
                if (deb > EB_DEB_T) {                  // ждём EB_DEB_TIME
                    ...
                    set_bf(EB_REL | EB_REL_R);         // флаг на отпуск
